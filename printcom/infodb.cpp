...
//
 ...
 //
 CStringW CStrA2CStrW_s(const CStringA &cstrSrcA)
 {
 ...
 //
 static CStringA CStrW2CStrA_s(const CStringW &cstrSrcW)
 {
...
CInfoDB::CInfoDB()
{
	strcpy(m_DBFile, "data.db");
	m_Table = g_Table;
	m_Column_Map = g_Column_Map;
}
...
int CInfoDB::CallBackFun(void *pUsed, int argc, char **argv, char **azColName){
...
int CInfoDB::InitDB()
{
	...
	ret = sqlite3_open(m_DBFile, &m_db);
	if(ret){
		sqlite3_close(m_db);
		return OPEN_DB_FAILED;
	}
				
	sprintf(sqlstr, "select * from %s", m_Table.c_str());
	ret = sqlite3_exec(m_db, sqlstr, NULL, NULL, &errmsg);
	if( ret != SQLITE_OK ){
		if(memcmp(errmsg, "no such table", strlen("no such table")) == 0){
			//Needed to create table
			sprintf(sqlstr, "create table %s (C0, C1 PRIMARY KEY, C2, C3, C4, C5, C6, C7, C8)", m_Table.c_str());
			sqlite3_free(errmsg);
			ret = sqlite3_exec(m_db, sqlstr, NULL, NULL, &errmsg);
			if( ret != SQLITE_OK ){
				sqlite3_free(errmsg);
...
int CInfoDB::ExecSQL(string sql)
{
	char *errmsg = 0;
	int ret = sqlite3_exec(m_db, sql.c_str(), NULL, NULL, &errmsg);
	if( ret != SQLITE_OK ){
		sqlite3_free(errmsg);
		return EXEC_SQL_FAILED;
	}
	return SUCCESS;
}

int CInfoDB::SetDBFile(char *file)
{
	memcpy(m_DBFile, file, strlen(file));
	return SUCCESS;
}

int CInfoDB::Query(string query, int(*callback)(void *, int, char**, char**),  void *pUser)
{
	int flag = 0;//0 means ASCII, 1 means ZH-CN
	unsigned char c;
	unsigned int d;
	string text;
	wstring wtext;
	char pStr[256];
	strcpy(pStr, query.c_str());
	if(query.size() > 0){
		c = query[0];
		d = c;
		if(d > 127) flag = 1;
	...
	if(flag == 0){//按索引搜索
		char *errmsg = 0;
		sprintf(sqlstr, "select * from %s where C0 like '%s%%'", m_Table.c_str(), query.c_str());
		int ret = sqlite3_exec(m_db, sqlstr, callback, pUser, &errmsg);
		if( ret != SQLITE_OK ){
			sqlite3_free(errmsg);
			return QUERY_DB_FAILED;
		}
	}else{//按公司名搜索
		char *errmsg = 0;
		m_Table = ASCII2UTF_8(m_Table);
		query = ASCII2UTF_8(query);
		sprintf(sqlstr, "select * from %s where C1 like '%%%s%%'", m_Table.c_str(), query.c_str());
		int ret = sqlite3_exec(m_db, sqlstr, callback, pUser, &errmsg);
		if( ret != SQLITE_OK ){
			sqlite3_free(errmsg);
			return QUERY_DB_FAILED;
...
int CInfoDB::QueryCompanyCallback(void *pUser, int argc, char **argv, char **azColName)
{
	CInfoDB *db = (CInfoDB*)pUser;
	db->QueryCompanyFlag = 1;
	db->m_TmpFindKey = argv[0];
	return SUCCESS;
}

int CInfoDB::QueryByCompany(string query, int(*callback)(void *, int, char**, char**),  void *pUser)
{
	QueryCompanyFlag = 0;
	char *errmsg = 0;
	query = ASCII2UTF_8(query);
	sprintf(sqlstr, "select * from %s where C1 = '%s'", m_Table.c_str(), query.c_str());
	int ret = sqlite3_exec(m_db, sqlstr, callback, pUser, &errmsg);
	if( ret != SQLITE_OK ){
		sqlite3_free(errmsg);
		return QUERY_DB_FAILED;
	}
	if(QueryCompanyFlag == 1){
		return SUCCESS;
	}
	return ERR;
}

int CInfoDB::Insert(string findkey, string company, string receiver, string tel, string address, string postalcode, string city)
{
	char *errmsg = 0;
	char timestr[16];
	if(company.empty()){
		return CANNOT_NOT_NULL_STR;
	}

	//findkey = ASCII2UTF_8(findkey);
	findkey.clear();
	getLetter(company, findkey);
	CharLowerBuffA((LPSTR)findkey.c_str(), findkey.size());
	company = ASCII2UTF_8(company);
	receiver = ASCII2UTF_8(receiver);
	tel = ASCII2UTF_8(tel);
	address = ASCII2UTF_8(address);
	postalcode = ASCII2UTF_8(postalcode);
	city = ASCII2UTF_8(city);
	CTime now;	
	now = CTime::GetCurrentTime();
	sprintf(timestr, "%04d.%02d.%02d", now.GetYear(), now.GetMonth(), now.GetDay());
	sprintf(sqlstr, "insert into %s (C0, C1, C2, C3, C4, C5, C6, C7) values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
		m_Table.c_str(), findkey.c_str(), company.c_str(), receiver.c_str(), tel.c_str(), address.c_str(), postalcode.c_str(), city.c_str(), timestr);
	int ret = sqlite3_exec(m_db, sqlstr, NULL, NULL, &errmsg);
	if( ret != SQLITE_OK ){
		sqlite3_free(errmsg);
		return INSERT_DB_FAILED;
	}
	return SUCCESS;
}

int CInfoDB::Delete(string company)
{
	char *errmsg = 0;
	company = ASCII2UTF_8(company);
	sprintf(sqlstr, "delete from %s where C1 = '%s'", m_Table.c_str(), company.c_str());

	int ret = sqlite3_exec(m_db, sqlstr, NULL, NULL, &errmsg);
	if( ret != SQLITE_OK ){
		sqlite3_free(errmsg);
		return DELELTE_DB_FAILED;
	}
	return SUCCESS;
}

int CInfoDB::Modify(string findkey, string company, string receiver, string tel, string address, string postalcode, string city)
{
	char timestr[16];
	CString str;
	CStringA strA;
	...
	if(Delete(company) != 0){
		return MODIFY_SQL_FAILED;
	}
	if(Insert(m_TmpFindKey, company, receiver, tel, address, postalcode, city) != 0){
		return MODIFY_SQL_FAILED;
	}
...
int CInfoDB::GetIndex(string company,  int(*callback)(void *, int, char**, char**),  void *pUser)
{	
	char *errmsg = 0;
	m_Table = ASCII2UTF_8(m_Table);
	company = ASCII2UTF_8(company);
	sprintf(sqlstr, "select C0 from %s where C1 like '%s'", m_Table.c_str(), company.c_str());
	int ret = sqlite3_exec(m_db, sqlstr, callback, pUser, &errmsg);
	if( ret != SQLITE_OK ){
		sqlite3_free(errmsg);
		return QUERY_DB_FAILED;
...
int CInfoDB::UpdateIndex(string findkey, string company, string receiver, string tel, string address, string postalcode, string city)
{
	char timestr[16];
	CString str;
	CStringA strA;
...
string getLetter(string strText, string &acronym)
{
	...
	for (int i=0; i<strText.length(); i++)
	{
		...
		strValue += strRes;
		strValue += " ";
		if(strRes.size() > 0) acronym += strRes[0];
		i++;
		if(m_LetterEnd) strValue=strValue+'\'';
	}
	return strValue;
